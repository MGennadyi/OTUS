...
Таблицы:
    "public.accounts"
```
###### На CITUS3 создание ПОДПИСКИ с переносом начальных данных с CITUS1:
```
CREATE SUBSCRIPTION accounts_sub 
CONNECTION 'host=192.168.5.177 port=5432 user=postgres password=12345 dbname=bank' 
PUBLICATION accounts_pub WITH (copy_data = true);
