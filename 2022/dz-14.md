...
######  1.9 Для шардирования по ключу для таблиц создается PRIMARY KEY (id); БД на каждой ноде создаем сами.
```
sudo -i -u postgres psql
\c bank
CREATE TABLE test2(i int);
# После создания таблицы неоюходим ключ, по которому будем разъезжаться:
# На координаторе:
...
bank=# SELECT * FROM master_add_node('citus4', 5432);
 master_add_node
-----------------
               6
(1 row)
