...
10.128.0.55 gp4.ru-central1.internal gp5
```
###### Правим репозиторий:
```
...
sh-5.0$
echo $shell
# Не помогло сменить dash на bash, т.к теперь идет запуск shell. Меняем shell на bash вручную:
chsh -s /bin/bash
Password: gpadmin123
# Ответ:
...
Если в домашнем каталоге gpadmin, то: 
vim ~/hostfile_exkeys
# иначе:
...
###### На всех хостах:
```
# Создаем конфиг с указанием сегментов:
cd ~
