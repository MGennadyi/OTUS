...
##### 0. Удаление ETCD:
```
# Если etcd установлен, но надо удалить:
apt purge etcd -y
...
```
```
# Сименов Алексей  ставил psycopg2
sudo apt-get install -y gnupg1 gnupg2 psycopg2
...
# Postgres, как самостоятельный сервис больше не должен запускаться:
...
# Делаем сим-линк: уточнить, до удаления или после?? :
...
#### Стартуем PATRONI как сервис:
```
systemctl daemon-reload
sudo systemctl start patroni
sudo systemctl is-enabled patroni
# Ответ: disabled. Поэтому делаем его :
