...
sudo wget --quiet -O - https://www.postgresql.org/media/keys/ACCC4CF8.asc | sudo apt-key add -
sudo apt update && sudo apt upgrade -y
sudo apt install sudo apt install -y postgresql-client-common postgresql-client -y
...
#### KEEPALIVED
###### 1. На haproxy1, haproxy2 Уст. и вкл вирт.сетевой адаптер, по умолч. откл:
```
...
# указываем  virtual_ipadress:
192.168.5.170
# вирт. роутер id должны совпадать.
sudo service keepalived start
sudo service keepalived status
```
```
# Ответ haproxy1
июл 04 13:02:01 haproxy1 Keepalived_vrrp[1811]: (VI_01) Entering MASTER STATE
```
```
# Ответ haproxy2
июл 04 13:02:01 haproxy2 Keepalived_vrrp[1833]: (VI_01) Entering BACKUP STATE
