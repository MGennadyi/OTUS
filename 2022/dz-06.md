...
```
# Установим последнюю версию posthresql. Должна установиться 14-я версия:
sudo sh -c 'echo "deb http://apt.postgresql.org/pub/repos/apt $(lsb_release -cs)-pgdg main" > /etc/apt/sources.list.d/pgdg.list'
wget --quiet -O - https://www.postgresql.org/media/keys/ACCC4CF8.asc | sudo apt-key add -
sudo apt update
apt install postgresql-14 -y
# Проверка установки:
systemctl status postgrsql
pg_lsclusters
pg_isready
# Ответ:
...
```
##### 2. Установка PATRONI
###### Запуск в 2-х вариантах:
```
# 1. Запуск бинарника с указанием конфига:
...
sudo sh -c 'echo "deb http://apt.postgresql.org/pub/repos/apt $(lsb_release -cs)-pgdg main" > /etc/apt/sources.list.d/pgdg.list'
# Появится запись: deb http://apt.postgresql.org/pub/repos/apt/ bullseye-pgdg main
...
# 2. Уст. ПО питон на 1 ноде:
sudo apt-get install -y python3 python3-pip git
# Уст. модули для pyton:
...
# Проверим остановленный кластер:
pg_lsclusters
# Ответ: 
Ver Cluster Port Status Owner    Data directory              Log file
14  main    5432 down   postgres /var/lib/postgresql/14/main /var/log/postgresql/postgresql-14-main.log
```
###### 3.2 Удаляем кластер 14 main:
```
...
```
##### Уст Patroni с модулем etcd. при старте сам создаст кластер:
```
...
# Ответ:                ERROR: Failed to get list of machines from http://etcd1:2379/v2: 
...
```
#### 1. Запуск PATRONI как сервис:
```
# т.к. patroni по умолчанию disabled, проверим это:
