...
# указываем  в keepalived.conf и keepalived2.conf virtual_ipadress: 192.168.5.180
...
```
```
psql -p 6432 -d otus -h 192.168.5.180 -U postgres
# Ответ psql: ошибка: подключиться к серверу "192.168.5.180", порту 6432 не удалось: В соединении отказано
