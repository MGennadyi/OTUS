...
git commit -m 'my fix'
git checkout master
git diff # -показывает разницу между последним коммитом и еще не добавленными в stage файлами
```
### Создание и переключение на ветку бранча под именем polaris
```
git switch -c polaris # этот бренч не планируем возвращать в мастер
...
### фича бренч 
```
git switch -c recipe02 # -создаю новый бранч и переключаюсь на него
